...
def serialize_decision_tree(model):
    ...
    serialized_model.update({k: (v.tolist() if isinstance(v, np.ndarray) else int(v) if isinstance(v, np.int64) else v) for k, v in vars(model).items() })
...
def serialize_random_forest(model):
    ...
    serialized_model.update({k: (v.tolist() if isinstance(v, np.ndarray) else v) for k, v in vars(model).items()})
    serialized_model["params"] = model.get_params()
    ...
    serialized_model['estimators_'] = [serialize_decision_tree(decision_tree) for decision_tree in model.estimators_]
